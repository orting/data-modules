...
class HRFDataModule(RetinaDataModule):
    ...
    def __init__(self,
        ...
        data_info_path : str
          EITHER
           an existing csv file containing *at least* the following named columns
             dataset,image_path,fov_path,annotation_path
        ...
        download : bool, optional
         ...
         prepare_data_for_processing : bool, optional
          If True, rename *.JPG to *.jpg, convert tif to png
    ...
    def prepare_data(self, predict=False):
        '''Do the following
        1. Download data
        2. Rename *JPG to jpg, convert *tiff masks to *png
    ...
    def _prepare_data_for_processing(self):
        ...
        print('Converting annotations to png')
        for entry in os.scandir(annotation_dir):
            if entry.is_file() and entry.name[-3:] == 'tif':
                annotation = (imread(entry.path) > 0).astype('uint8')
        ...
        for entry in os.scandir(fov_dir):
            if entry.is_file() and entry.name[-3:] == 'tif':
                fov = (imread(entry.path)[...,0] > 0).astype('uint8') # They are stored as RGB
    ...
    def _create_data_info(self, predict=False):
        ...
        with open(self.data_info_path, 'w', newline='', encoding='utf-8') as outfile:
            ...
            writer.writerow(['dataset', 'image_path', 'fov_path', 'annotation_path'])
            writer.writerows(zip(datasets, image_paths, fov_paths, annotation_paths))

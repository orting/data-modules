...
from .retina_data_module_v2 import RetinaDataModule
...
class STAREDataModule(RetinaDataModule):
    ...
    def __init__(self,
        ...
        data_info_path : str
          EITHER
           an existing csv file containing *at least* the following named columns
             dataset,image_path,annotation_path
           all other columns are ignored.
           The dataset column must contain values from 
             {train, unlabeled-train, validation, test, predict}.
          OR
        ...
        annotation_merge_style : one of {'bitmask', 'union', 'intersection', 'ah', 'vk'}
    ...
    def prepare_data(self):
        '''Do the following
        1. Download data
        2. Convert ppm to png, merge_annotations
    ...
    def _prepare_data_for_processing(self):
        ...
        assert self.annotation_merge_style in ('bitmask', 'union', 'intersection', 'ah', 'vk')
        
        image_dir = os.path.join(self.data_dir, 'images')
        label_dir_ah = os.path.join(self.data_dir, 'labels-ah')
        label_dir_vk = os.path.join(self.data_dir, 'labels-vk')
        annotation_dir = os.path.join(self.data_dir, 'labels')
        os.makedirs(annotation_dir, exist_ok=True)

        for image_id in self.labeled + self.unlabeled:
            image_name = f'im{image_id:04d}'
            ppm_path = os.path.join(image_dir, image_name + '.ppm')
            png_path = os.path.join(image_dir, image_name + '.png')
            image = imread(ppm_path)
            imsave(png_path, image, check_contrast=False)

        for image_id in self.labeled:
            image_name = f'im{image_id:04d}'
            ah_path = os.path.join(label_dir_ah, f'{image_name}.ah.ppm' )
            vk_path = os.path.join(label_dir_vk, f'{image_name}.vk.ppm' )
            ...
            elif self.annotation_merge_style == 'intersection':
                mask = np.logical_and(ah_mask, vk_mask).astype('uint8')
            elif self.annotation_merge_style == 'ah':
                mask = ah_mask
            elif self.annotation_merge_style == 'vk':
                mask = vk_mask
            annotation_path = os.path.join(annotation_dir,
                                           f'{image_name}.{self.annotation_merge_style}.png')
    ...
    def _create_data_info(self):
        ...
        datasets = ['unlabeled-train']*n_unlabeled + ['train']*n_train + ['validation']*n_val + ['test']*n_test
        
        image_paths, annotation_paths = [], []
        image_dir = os.path.join(self.data_dir, 'images')
        annotation_dir = os.path.join(self.data_dir, 'labels')        
        for image_id in self.unlabeled: 
            image_name = f'im{image_id:04d}'
        ...
        for image_id in self.labeled: 
            ...
            image_paths.append(os.path.join(image_dir, f'{image_name}.png'))            
            annotation_paths.append(
                os.path.join(annotation_dir, f'{image_name}.{self.annotation_merge_style}.png')
            )            
            
        with open(self.data_info_path, 'w', newline='', encoding='utf-8') as outfile:
            writer = csv.writer(outfile)
            writer.writerow(['dataset', 'image_path', 'annotation_path'])
            writer.writerows(zip(datasets, image_paths, annotation_paths))

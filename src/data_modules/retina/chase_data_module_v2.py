...
class CHASEDB1DataModule(RetinaDataModule):
    ...
    def __init__(self,
        ...
        annotation_merge_style : one of {'bitmask', 'union', 'intersection'}
          ...
          '1st' : Only use 1st annotator
          '2nd' : Only use 2nd annotator

        create_fov : bool, optional
          If True, create a field of view mask using thresholding
    ...
    def _prepare_data_for_processing(self):
        print(f'Merging annotations{" and creating field-of-view" if self.create_fov else ""}')
        for side in ('L', 'R'):
            for image_number in range(1, 15):
                ...
                if self.create_fov:
                    ...
                    fov_path = os.path.join(
                        self.data_dir,
                        f'Image_{image_number:02d}{side}_fov.png'
                    )
                ...
                elif self.annotation_merge_style == '1st':
                    annotation = ann1st.astype('uint8')
                elif self.annotation_merge_style == '2nd':
                    annotation = ann2nd.astype('uint8')
                annotation_path = \
    ...
    def _create_data_info(self):
        ...
        image_paths, fov_paths, annotation_paths = [], [], []
        for side in ('L', 'R'):
            for image_number in range(1, 15):
                image_paths.append(
        ...
        with open(self.data_info_path, 'w', newline='', encoding='utf-8') as outfile:
            ...
            if self.create_fov:
                writer.writerow(['dataset', 'image_path', 'annotation_path', 'fov_path'])
                writer.writerows(zip(datasets, image_paths, annotation_paths, fov_paths))
            else:
